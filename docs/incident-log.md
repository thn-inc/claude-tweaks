...
**Correction (record #1800, 2026-09-14):** the `#989` exemption (`hasNoUpstreamYet` — lets an in-flight initial publish push through the PR-stamp branch unconditionally, since that push *is* `pr-early-run-lifecycle.md` Step 2 itself) never checked that Step 1 — the `gh pr list --head {branch} --state all` reuse/reopen check that precedes it — actually ran first. In dispatch run `2026-09-03T041741-record-903`, the first Task call skipped both stamps (the same IL-131 trigger); the second call's test phase-exit push was that branch's first push, was exempted by `#989` unconditionally, and collided with a stale, still-open draft PR from an earlier tombstoned attempt at the same record (non-fast-forward against unrelated history), needing a manual force-push and reconcile — the exact collision Step 1's reuse/reopen check exists to prevent, bypassed because the exemption asked "is this the branch's first push?" without asking "did Step 1 run first?". Root cause: Step 1 logged a `decisions.md` line on its reopen outcome only, never on its no-match or reuse-open outcomes, so a gate could not tell "Step 1 ran and found nothing" from "Step 1 never ran." Fixed by making all three Step 1 outcomes log a `PR-early run lifecycle:` line (`_shared/pr-early-run-lifecycle.md`), and tightening the `#989` exemption to require that branch-scoped line in `decisions.md` before letting the push through — absent it, a new `record-pr-step1` deny fires, naming Step 1's check. One-shot and unpersisted, matching `#989`'s own discipline; a resolved `local-merge` run is unaffected, since it never runs Step 1 at all.
...
## IL-158 — A regression test's cited historical incident was never ground-truthed against its actual resolved state
