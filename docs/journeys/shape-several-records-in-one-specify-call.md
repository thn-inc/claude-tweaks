...
**Entry point:** A Claude Code session at the project checkout, `work-backend: github-issues`, the record numbers in hand — typed from memory, or pasted straight from a `/tidy` report's Yours section (`/claude-tweaks:specify` shows the grammar `[#N[,#M...]|#A-#B|record-id[,id...]|design-doc-path|topic|backlog-title] [--budget <n|all>] …` as the greyed argument hint, and `/claude-tweaks:help`'s reference card carries the same string).
**Success state:** Every record in the batch is `ready` with `risk:*`/`size:*`/`ceremony:*` stamped, its body carries `Surface:` + the six spec sections (Current State, Deliverables, Acceptance Criteria, `## Release Note` — required on every record since #2580, Technical Approach, Gotchas) + `## Original request`, the Actions Performed table shows one row per record, and the terminal `## Next Actions` block leads with **`/claude-tweaks:flow #701,#702,#703`** — the maintainer never re-derived a command by hand.
...
- **Should understand:** Every element resolves independently (parallel fetches, as `flow/materialize.md`'s Resolution does); if any element cannot be resolved, all unresolvable elements are reported in one message before anything is shaped. A comma list is shaping-mode-only — decomposition (a design doc) and topic resolution stay single-input. The range form is capped at 25 elements (a hard input error names the element count above that) and requires `A ≤ B`; a range collapsing to one element (`A == B`) resolves as an ordinary single record reference, not through the batch path. A bare invocation with no record reference is a different, headless form entirely — it drains the queue via `--budget` (default one record, `all` for the whole eligible set) chosen by the skill rather than any record the user names, and is mutually exclusive with the comma-list and range forms documented here; the pre-#1491 literal `next` argument is a deprecated alias for `--budget 1`, no longer shown in the argument hint.
...
- **Should feel:** Complete at a glance — every record has a row naming what was stamped (`risk:`/`size:`/`ceremony:`, Type where absent, `ready` added, `parked` and every `needs:*`-prefixed label removed if present, `breaking` added when the shaped Acceptance Criteria name a contract change — #2251), and every record was re-fetched and verified immediately after its own write — not just written and hoped for.
- **Should understand:** Each record was written by its own compose-then-write-once call, then immediately read back (a fresh `gh issue view`/`readRecord` re-fetch, never trusting the write call's own response) to assert `ready` + labels are present, the six spec sections (Release Note included since #2580 — `_shared/work-record.md`'s spec-shaped-body check now names it a required, non-empty section alongside Current State/Deliverables/Acceptance Criteria) + `## Original request` are present, a `breaking`-stamped record additionally carries a non-empty `## Breaking Change` section (a `breaking` record without one fails `bin/compose-subject.js` at merge time, so the read-back is where it is caught — #2251), `parked` and every `needs:*` label the record carried are absent (a `needs:decision` removal also closes its live decision comment with a `**Resolved:**` line in the same batch iteration), and no placeholder marker survived outside the verbatim-preserved `## Original request` copy (markers inherited there are sanctioned — #1240). A failure shaping record *k* — whether the write itself or its read-back — does not roll back records 1..k-1 — that record's row carries `failed` in the Detail cell and the rest still shape. Under `work-backend: local-files` there is one commit per record. A record whose `needs:decision`-resolving comment edit fails is a legitimate exception to "removed if present": `shaping-mode.md` fails closed and leaves the label in place rather than removing it over an unresolved comment, so that record's row reports the retry, not a clean pass. A third Detail-cell outcome exists alongside `shaped`/`already shaped, no-op`/`failed`: `refused — proposed Absorb into #{candidate}` — the near-duplicate candidate check (#1944) found a `ready`, in-flight-build candidate for this record and stopped before composing, stamping `needs:decision` instead of `ready`. Same per-record failure-isolation posture as a write/read-back `failed` row: the rest of the batch keeps shaping. When a record's Key Files names a file that another module generates (the generated-file grep in `spec-template.md`'s `### Key Files` guidance — #326's incident), the composed body lists that generator module as the primary edit target and the generated entry's own bullet carries a `generated by` annotation — so a shaped record never again points a build at a generated file without its source of truth.
- **Red flags:** A single collapsed row for the whole batch; a failed record silently missing from the table; a record whose write succeeded but whose labels or sections didn't actually land, with no row flagging it; a `needs:decision` label removed from a record whose decision comment failed to resolve; a near-duplicate-refused record composed and stamped `ready` anyway; a record stamped `breaking` whose composed body carries no `## Breaking Change` section.
...
## Origin
- Step 3 updated for #1944 (near-duplicate candidate check: a third Detail-cell outcome, `refused — proposed Absorb into #{candidate}`, isolated per-record like a write/read-back failure)
- Step 3 updated for #1321 (generated-file grep: a shaped record's Key Files lists a generated entry's generator as primary edit target, with the entry annotated)
...
- Entry point and Step 1 updated for #1491 (bare drain mode: the argument hint's literal `next` alternative was replaced by a bare invocation plus `--budget`; `next` survives only as a deprecated alias)
- Step 3 updated for #2251 (Compatibility axis — shaping mode stamps `breaking` from the Acceptance Criteria’s contract-change language, and the read-back asserts the composed body’s `## Breaking Change` section)
- Success state and Step 3 updated during wrap-up of #2580 (Release notes: authoring contract) — `spec-template.md` gained a required `## Release Note` section and `_shared/work-record.md`'s spec-shaped-body check grew from three to four required sections, so "the five spec sections" the read-back verifies is now six
- Related specs: #1071, #705, #695/#702 (comma-list batch form and this journey's original steps), #357, #822, #1488, #1491
