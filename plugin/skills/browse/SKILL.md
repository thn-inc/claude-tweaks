...
> source. Each agent's reply must open with the chosen template verbatim, then end with its
> last non-empty line reading exactly `STATUS: DONE` (or DONE_WITH_CONCERNS / NEEDS_CONTEXT /
> BLOCKED).
