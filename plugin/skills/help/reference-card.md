...
| `/claude-tweaks:feedback` | Route a learning upstream to the claude-tweaks plugin — defect or gap. | `[<learning text>] [--kind=defect\|gap] [--upstream <owner/name>] [--dry-run] [--queue] [--full] [--pre-confirmed]` |
...
| `/claude-tweaks:dispatch` | The queue consumer — selects an authorized record's whole file-overlap group, mints its run directory, and hands it to `/flow` (which claims it via an atomic blob lock at its Step 2.8); settles on success/failure. `close-out` is a distinct mode: closes out an already-`pending-review` PR a human approved directly on GitHub, with no live dispatch session left to resume | `[#N[,#M...]] [--budget <n\|all>] [--priority high\|medium\|low] [--settle-parked] [close-out [#N[,#M...]]]` |
