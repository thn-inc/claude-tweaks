...
| `/feedback` | A described defect or gap in a claude-tweaks skill, or (via `--upstream <owner/name>`) in a third-party dependency | A GitHub issue against `thomasholknielsen/claude-tweaks` (human-invoked, after explicit scrub + confirmation); or, via `--upstream`, a scrubbed draft file staged for the human to paste elsewhere — never published | — |
