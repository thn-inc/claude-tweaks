...
Before composing anything, check the fetched body against `_shared/work-record.md`'s spec-shaped body definition: the sections `## Current State`, `## Deliverables`, and `## Acceptance Criteria` are present and each non-empty, and no unresolved placeholder marker (`TBD`, `TODO`, `<!-- ambiguity:`) remains anywhere outside the verbatim-preserved `## Original request` section (that heading to end of body is exempt — see that definition's #1240 clause). This build-time gate is deliberately narrower than `_shared/work-record.md`'s "Spec-shaped body" section as of #2580: that section's `ready`-label check also requires `## Release Note`, but this materialize-time gate does not — a record missing only `## Release Note` should still build, and is caught instead at merge time by `compose-subject.js`'s unconditional gate (`_shared/work-record.md`'s "Labels are projection, not truth" note).
