...
**`bin/materialize.js <n> --run-dir <dir> [--repo owner/name] [--ceremony fast-lane|standard] [--multi-record-slug <n>] [--record-json <path>]`** implements this file's resolution (`work-backend: github-issues`), the Materialization hard gate, header composition (`bin/lib/issues/materialize-format.js`), and the write, mechanically — `/flow`/`/build` invoke it for `github-issues` records instead of hand-composing the header inline per run. `--ceremony` is required only as a fallback when the record carries no `ceremony:*` label (the CLI cannot itself invoke the `assess-agent-autonomy` `ceremony-check` LLM judgment call below — the caller runs that first and passes the verdict through). `work-backend: local-files` is not yet wired into this CLI; that driver still reads inline per the procedure below. **`--run-dir` is a literal write-location prefix, not a run identity** — the CLI writes `{--run-dir}/work/{n}-spec.md` exactly under whatever path is passed, with no worktree-awareness of its own. In `worktree` mode (the "Sequence" below), invoke this from inside the already-created worktree and pass the **worktree-local** path to the run directory — substitute the worktree's own filesystem root for `$RUN_ROOT` in the otherwise-`$RUN_ROOT`-anchored `$PIPELINE_RUN_DIR` value — never the bare `$RUN_ROOT`-anchored value itself; passing that one writes `work/` into the main checkout instead of the worktree, defeating the worktree-first ordering this section exists to enforce.

**`--run-dir`/`--multi-record-slug` interaction (#2294).** These two flags must never both descend into the same per-spec level — `--run-dir` is the literal write-location prefix above, and `--multi-record-slug <n>` (when passed) appends one more `spec-{n}/` level beneath it (`{--run-dir}/spec-{n}/work/{n}-spec.md`), never two. Which of the two forms applies depends on what `--run-dir` already names at the call site:

- **`--run-dir` is the shared *parent* directory** (`/claude-tweaks:dispatch`'s bundled-group Task calls — `dispatch/task-prompt.md`'s row — where the minted run dir names the whole file-overlap group, not one record) — pass `--multi-record-slug <n>` so the CLI does the one per-spec descent: `{minted-run-dir}/spec-{n}/work/{n}-spec.md`.
- **`--run-dir` already resolves to a per-spec `{parent}/spec-{N}/` directory** (`/claude-tweaks:flow`'s own internal multi-spec sequential mode — `multi-spec.md`'s `PIPELINE_RUN_DIR` table entry, `{parent}/spec-{N}/`, exported once per record before that record's own `/build` runs) — do **not** also pass `--multi-record-slug`; the per-spec descent already happened when `PIPELINE_RUN_DIR` was resolved, and doing it again produces `{parent}/spec-{N}/spec-{N}/work/{N}-spec.md`. `/flow`'s own multi-spec sequence follows this rule today: each record's `/build` Common Step 1 materializes with `--run-dir "$PIPELINE_RUN_DIR"` (already per-spec) and no `--multi-record-slug`.

A caller that is unsure which case it is in should ask: does `--run-dir` already end in `spec-{this-record}/`? If yes, `--multi-record-slug` is redundant and wrong; if `--run-dir` is a directory shared by multiple records, `--multi-record-slug` is required to keep them from colliding on one `work/` directory.

**`--record-json <path>` (#1459) is the `gh`-absent input path.** This CLI is a plain Node subprocess and cannot itself call an MCP tool — a node subprocess cannot see an agent session's MCP tools, the same platform constraint `release-claim.js`'s header and `reconcile/pr-state.js` state. When `gh` is unavailable (a cloud/Routine sandbox with GitHub MCP tools instead), the **calling skill** — which runs inside the LLM harness and already has MCP access in this case — fetches the record itself via MCP `issue_read` (get mode, `_shared/github-write-transport.md`'s existing read-side CRUD-mapping row), writes the returned JSON (reshaped to the same `{number,title,body,labels,url}` shape `gh issue view {n} --json number,title,body,labels,url` returns — `labels` as the array-of-`{name}`/bare-string shape `parseRecordFacets` expects — plus an optional `authorAssociation` string field when the record carries a `Premise-check:` line and the caller wants that check to actually run; see the "Author-association gate" paragraph below) to a temp file, and invokes `node bin/materialize.js <n> --record-json <file> --run-dir <dir>` instead of relying on `ghView`. `deps.ghAvailable()` is never consulted on this path. Everything downstream of the record fetch (shape gate, drift, header composition, write) is unchanged, since it already operates on the parsed record object regardless of how it arrived.
...
**Named-location drift (#315).** The stamp above bounds a premise's *age*; it says nothing about whether the locations the body *names* still exist, and `drift` is `null` for every record carrying no stamp — which is every record not filed by a health sweep. Before scoping an implementation to a file, function, or step the record's prose names, confirm that location still exists in this checkout (grep the named symbol, `ls` the named path), and treat the stated address as a hypothesis to verify rather than a fact to build on — except a `by:docs-health` record's `**Doc:**` field (#1851): that path is repo-relative and resolvable as written (`docs/{id}.md`), so it needs only an `ls`, not a grep to infer it from the registry id. Record bodies are never rewritten when a later refactor moves what they describe, and a relocation sweep reaches docs and skills but not the open backlog: #464 moved claim acquisition out of dispatch into `/flow`'s Step 2.8 and swept the stale "dispatch claims" attribution from README/docs, yet #315 — filed under the old model — still named "Dispatch's Step 4 claim path" as the fix site in seven places when it was finally built, and the first implementer scoped the fix to that stale module before a second dispatch corrected it.

**Premise check (#1829).** The two drift checks above can only say a premise *might* be stale (age, or a named location that might have moved) — neither positively confirms the record's Current State claim no longer holds. When the record's body carries an optional `Premise-check: {command}` line (a plain body-metadata line, same convention as `Verified-as-of:` — written by a filing site that knows its own mechanical check, e.g. `wrap-up/claude-md-curation.md`'s over-budget filing composing `test $(wc -l < {path}) -gt {budget}`), the CLI runs that command from the checkout root and includes the result in the JSON envelope as `premise: { command, exit, satisfiedAtBase }` — `satisfiedAtBase: true` when the command exits non-zero (the command is written to exit 0 while the premise still holds, so a non-zero exit means it no longer does); `premise` is `null` when the record carries no `Premise-check:` line, the check was gated out (below), or the command could not be run at all (never a crash). When `satisfiedAtBase` is true, the CLI also prints "premise already satisfied at base: `{command}` exited {n}" to stderr and, under `--run-dir`, logs a `STAGED` decision plus a staged close-proposal note (`staged/premise-satisfied-{n}.md`) for the Review Console — never a silent proceed, and never a hard stop from the CLI itself. `/build`'s Spec Step 2 reads `premise.satisfiedAtBase` from the envelope and routes it to the Console's staged close rather than planning a build — see that skill's Step 2.

**Author-association gate (security fix from a whole-branch pre-release review).** A `Premise-check:` line is body text — anyone who can create or edit the issue can write one, regardless of who actually composed it via `specShapedBody`. The CLI never runs the command unless GitHub itself attests the issue author is trusted: on the `gh`-backed path, a `gh api repos/{owner}/{repo}/issues/{n}` lookup of `author_association`, trusted only at `OWNER`/`MEMBER`/`COLLABORATOR`; on the `--record-json` path, the caller's own optional `authorAssociation` field on the supplied record (absent → untrusted by default — the calling skill must fetch and pass it through if it wants the check to run). An untrusted, unresolvable, or absent association degrades exactly like "no `Premise-check:` line" — `premise: null`, with a stderr note naming the association — never a hard stop.
...
Before composing anything, check the fetched body against `_shared/work-record.md`'s spec-shaped body definition: the sections `## Current State`, `## Deliverables`, `## Acceptance Criteria`, and `## Release Note` are present and each non-empty, and no unresolved placeholder marker (`TBD`, `TODO`, `<!-- ambiguity:`) remains anywhere outside the verbatim-preserved `## Original request` section (that heading to end of body is exempt — see that definition's #1240 clause) or inside a fenced code block / inline code span in an authored section (that definition's #1839 clause — a quoted marker is content, not an unresolved placeholder). As of #2603 this matches `_shared/work-record.md`'s "Spec-shaped body" section exactly (#2580) — a record missing only `## Release Note` is stopped here, at materialize time, the same way a missing `## Current State` already is, rather than only being caught later at merge time by `compose-subject.js`'s unconditional gate.
...
| `size` | `/build` size-based profile selection — caps at Capable (`low`→Fast / `medium`→Standard / `high`→Capable, `skills/build/SKILL.md` Common Step 2). Frontier is never derived from this field or any other header value — it is invocation-only, reachable solely via a `profile=frontier` token (or its `tier=frontier` alias) typed on the `/build`/`/flow` command line itself (`dispatch.md`'s canonical guard, read from Common Step 2). Consequently `profile=frontier` is never materialized into this header; the run dir's `frontier-tally.log` — not this file — carries Frontier cap state across the build's sequential dispatches, and survives interrupt/resume because the tally is append-only in the run dir. Second reader since #1911: `build/micro-plan.md`'s applicability gate reads `size: low` (alongside `ceremony: fast-lane` below) before Spec Step 3 decides whether to skip `/superpowers:writing-plans`. |
| `ceremony` | `/flow`'s Manifesto (Step 3) bundle-fold into the `ceremony-profile` lever. Second reader since #1911: `build/micro-plan.md`'s applicability gate reads `ceremony: fast-lane` from this same header, not the run's `config.yml` — the header carries the value even for a standalone build with no `config.yml` to fold into. |
...
- `blocked-by` — the record's dependency targets, driver/`work-links`-dependent: `work-backend: github-issues` + `work-links: body-text` — `parseDependencies(body)` (`bin/lib/issues/record.js`) over the already-fetched body, no extra read; `work-backend: github-issues` + `work-links: native` — one batched `gh api graphql` call across every record in the run (aliased per record number, reusing `buildNativeDependencyQuery`/`hasOpenNativeBlocker` from `bin/lib/issues/record.js` — the same builder and OPEN-state predicate `/claude-tweaks:dispatch` Step 2 and `wrap-up/unblocked-records.md` now reach through `bin/resolve-blockers.js`, the CLI wrapping `bin/lib/issues/native-dependencies.js`'s `fetchNativeDependencies`, rather than through a hand-rolled call of their own), resolving `blockedBy` (the field `capabilities-probe.js`'s `probeSchema` checks for), added to Resolution. A single-record run is the degenerate one-alias case of the same call; `work-backend: local-files` — `facets.blockedBy`, already present on the read record. Emit as `blocked-by: [n1, n2, ...]`; omit the line when empty. Resolution is read-only and safe to run before any run dir or worktree exists (see "When this runs" below), so this data is available to `/flow`'s multi-spec pre-flight (`multi-spec.md`'s "Frontmatter pre-flight") immediately after Resolution — it does not need to wait for the header to be composed and written to disk.
...
Sequence, in `worktree` mode (the default): create the worktree (`/build`'s Common Step 1, or `/flow`'s own up-front shared-worktree creation for a multi-record run). `{run-dir}` itself is never scaffolded inside the worktree — it is anchored at `$RUN_ROOT` per `_shared/pipeline-run-dir.md`'s Anchoring section (already created there by `/flow`'s own Step 3 Manifesto in the common case, or resolved via `_shared/run-dir-resolution.md`'s Resolution order otherwise). Only `work/` is created — inside the worktree, at the matching relative path under the worktree's own filesystem location — then compose the file and commit it there as the branch's first commit.
...
**Standalone fallback (no run dir resolves).** When `/claude-tweaks:build #{n}` runs standalone — no `/claude-tweaks:flow` parent, and neither the `PIPELINE_RUN_DIR` env var nor a most-recent matching directory resolves per `_shared/run-dir-resolution.md`'s Resolution order (steps 1-2) — create the standalone run dir's gitignored half (`decisions.md` + `staged/`, per `_shared/run-dir-resolution.md`'s standalone conventions) at `$RUN_ROOT/.claude-tweaks/pipelines/{ISO-timestamp}-record-{n}-standalone/` (`$RUN_ROOT` resolved per `_shared/pipeline-run-dir.md`'s Anchoring section, not the current directory), and materialize `work/{n}-spec.md` into the **worktree's** copy of that same relative path — per the Anchoring section, `work/{n}-spec.md` is the tracked exception that stays in the worktree, not under `$RUN_ROOT`, and reaches the main checkout only by merge. This is narrower than `_shared/run-dir-resolution.md`'s Resolution-order step 4 (the auto-mode standalone allowlist): it applies regardless of mode — materialization needs somewhere to write the file whether or not `auto` is active — and `/build` is not itself on that allowlist. (This same exception is now also its own numbered step 3 in `_shared/run-dir-resolution.md`'s Resolution order, folded into that algorithm rather than trailing prose after it.) The resulting standalone run dir becomes this build's `$PIPELINE_RUN_DIR` for the rest of the run (ledger, decisions, and cleanup all resolve against it the same as any other run dir).
...
Materialized files are meant to survive as ordinary tracked history on the branch that builds them. Unlike the rest of the pipeline run directory (`config.yml`, `decisions.md`, `staged/`, `run-state.json`, `events.jsonl` — all gitignored, archived as plain files at wrap-up, never committed for a `/flow`/`/build` run; a `*-tidy-standalone*` or `*-sweep-standalone*` run's own `decisions.md`/`report.md`/`staged/**` are the one carve-out to "never committed" — `_shared/auto-decision-log.md`'s tidy-standalone/sweep-standalone clause, #1493/#1494; see `_shared/auto-mode-contract.md` for everything else), the `work/` subdirectory is a deliberate, narrow exception: `.gitignore` carves it out of the otherwise-blanket `.claude-tweaks/pipelines/` ignore so `git add`/`git commit` actually track it, at both the single-record depth (`{run-id}/work/`) and the multi-record depth (`{run-id}/spec-{N}/work/`).
