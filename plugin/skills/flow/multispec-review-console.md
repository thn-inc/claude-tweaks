...
When granted, resolve every member run dir in one process each (#1932, #2007) — never a hand-rolled per-item loop. Render every section below as an informational report (nothing dropped): for each `spec-{N}/` subdirectory named in `manifest.yml` (spec execution order from Step 1), plus once more over the parent run dir itself, invoke:

```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/console-resolve.js" --run {dir} --policy console-auto [--dry-run]
```

`{dir}` is `{parent}/spec-{N}` for each manifest member, then `{parent}` itself, in that order — N+1 calls, never a single call over the whole tree: `bin/lib/console/resolve.js`'s `readMembers`/`readSnapshot` both operate on one flat run dir, and there is no parent-manifest-aware mode (rejected — the multi-spec skill file already owns manifest-reading, and duplicating it in the resolver would create a second place that can drift). Append `--dry-run` to every call when the run itself is `--dry-run`.

**Per-call isolation** mirrors Step 3's engine-call precedent above: a call that exits non-zero for one `spec-{N}/` — a malformed `engine-state.json`, or any of the CLI's own non-zero exits (2 malformed invocation, 3 `--run` not anchored under the main checkout, 4 `consoleAutoResolve` not granted for that directory's own resolved policy, 5 its `console.json` is already a rendered-on-PR console or unparseable) — does not abort the fan-out for the remaining directories. Note the dropped directory in the console's Not run/Failed footer with the CLI's own stderr text as the reason, and continue with the rest. The parent-level call failing is reported the same way, attributed to the parent run dir rather than a spec.

Insert each surviving call's stdout table verbatim into the console response, one render per spec-dir plus the parent's, in the same manifest order — do not hand-expand it. Each call already appends its own `AUTO` decisions block to that directory's own `decisions.md` and writes that directory's own `console.json` (`{resolved: true, mode: 'auto-resolve', items, merge, ...}` — the write `archive-merged.js`'s `readConsoleState` needs, per spec dir and the parent, closing the same gap #1854 closed for the single-spec path) — there is no separate per-item logging step here; the CLI already did it.

Then execute every returned resolution, across every surviving call, through this file's own "On approval" procedure below (each item routes back to the spec-dir it was resolved from) — batch sections and `Q#`/`M#` as if Approve all had been chosen; `U#` resolves to **filed unconditionally** — causal-tagged or not, unlike "Approve all"'s own conditional default (same rule the single-spec short-circuit states).

**The bundle-level branch-finish/merge decision is never read from any individual call's own `merge.resolution` field.** Each `console-resolve.js` invocation computes `merge` from only its own `--run` directory's members and grants — a spec-dir call cannot see the bundle's other members, and the parent-level call has no members at all (`readMembers` finds no `work/` directory at the parent, so its `merge.resolution` always reads `leave-open, reason: members-unresolved` — expected, and discarded either way). The bundle decision is computed independently, exactly as before: **the merge half of the terminal decision defaults to merge** (`integration-model: pr-first`'s "Approve all + merge" variant) — `unattended` authorizes full completion, not a park; `local-merge` proceeds to branch-finish the same way. **Needs-human carve-out (merge-check precedence):** when an `assess-agent-autonomy` `merge-check` verdict of `needs-human` exists for this run (any member's verdict, for a bundle — `dispatch/settle-and-merge.md`'s Auto-merge gate, Layer 2), the merge half instead resolves to **leave the PR open** (`pending-review`), never merge — `consoleAutoResolve`'s default-merge never overrides the more specific gate's routing to a human; mirrors the single-spec carve-out in `wrap-up/review-console.md`. **Ungranted-member carve-out (#1802):** the same leave-open resolution also applies when any group member's live labels lack both `auto:merge` and a matured `auto:merge-pending` — the Auto-merge gate's Layer 1 falls through before Layer 2's `merge-check` ever runs on a mixed-grant bundle, so no verdict exists to carry the precedence above; a withheld grant is itself a human decision and must never silently resolve to merge for want of a verdict that was never computed; mirrors the single-spec carve-out in `wrap-up/review-console.md`. **Drain-overlap-hold carve-out (#2299):** the same leave-open resolution also applies when `dispatch/drain-pr-overlap.md`'s own Auto-merge-gate hold (#1985) logged this group — or, for a bundle, any member group — as overlapping a still-open PR from earlier in the same drain firing; `console-resolve.js`'s `mergeResolution()` re-verifies that PR's live state and resolves leave-open only while it is still genuinely open, self-healing once that PR merges or closes; mirrors the single-spec carve-out in `wrap-up/review-console.md`. Every non-merge item still auto-resolves exactly as this section states.

Retain every `staged/` file as a revert artifact rather than consuming it (applied ones included), and send **one** consolidated `PushNotification` for the whole run — not per spec, not per item, not per call — at the same point the single-spec short-circuit sends its own FYI. Then proceed straight to Cleanup actions execution (Shared teardown below) and archive the parent run dir — skip only the `AskUserQuestion` prompt; already rendered above. End this paragraph's execution by printing the absolute path to every touched `decisions.md` and `console.json` (the parent's, plus each spec's own) and any retained `staged/*.md` files, so the operator has a concrete pointer even though nothing prompted at `unattended`.
...
- Option 1 — `label`: `"Approve all + merge (Recommended)"`, `description`: `"Apply pending items, accept auto-applied, apply skill + config updates, execute cleanup incl. merging the shared branch (items {N}-{M}); resolve every Q#/M# to Apply and every U# to filed (systemic causal tag) or declined (otherwise)."`
...
Queue writes (Q1, Q2, …), Memory updates (M1, M2, …), and Upstream feedback (U1, U2, …) all resolve as part of the terminal decision now, mirroring `wrap-up/review-console.md`'s single-spec stance exactly: choosing Approve all resolves every `Q#`/`M#` item to `Apply` and every `U#` item to filed (staged draft carries a `**Causal:** systemic` tag) or declined (otherwise) — their own stated defaults — with no further prompt. Choosing Override is what still drills them individually, below.
...
4. For `U#` upstream feedback rows: Approve all resolves each row per its own conditional default — filed, when the staged draft carries a `**Causal:** systemic` tag (`upstream-feedback.md`'s Step 2); declined otherwise — no prompt either way (the Auto-resolution short-circuit above still files every row unconditionally, tag or not); Override presents them via `_shared/upstream-feedback-batch.md`'s shared batch contract (above) — the rendered draft **is** the approved snapshot. For each item checked (Override), or resolved to filed by the Approve all default (the systemic-tagged case): invoke `/claude-tweaks:feedback --pre-confirmed` with the staged-file path and that snapshot for its drift check (Step 6 scrub always reruns regardless; on drift it falls back to its own confirm). An item left unchecked (Override), or resolved by the Approve all default without the tag, is declined per the shared contract's rule — log the decline the same way as `Q#`/`M#`.
...
4. **Remove grants** — pass `--remove-grants` for each issue released with a `merged:` or `pr-opened:` outcome (strips `auto:build`/`auto:merge-pending`/`auto:merge`, best-effort per label); omit it for `abandoned:` (the grant is the standing retry request). See "Grant revocation" in `_shared/issue-claims.md`.
