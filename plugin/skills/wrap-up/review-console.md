...
See `_shared/run-dir-resolution.md` for the resolution order and bash snippet. Resolution is unchanged; an empty result is **unreachable after Phase 1**, which creates a run directory on every run when none was inherited. If it happens anyway, do not skip the console — treat it as the prose-fallback case: present the same findings inline in this response, gathered from what this run itself produced rather than from `decisions.md` and `staged/`, and take the same terminal decision below.
...
Resolve the ceiling once — `CEILING=$(node "${CLAUDE_PLUGIN_ROOT}/bin/resolve-policy.js" --values --run "$PIPELINE_RUN_DIR" autonomy)` (the `--run` overlay is what makes this pre-check agree with the CLI below, which folds run config over project policy in its own resolve) — and check `bookkeepingPermissions(ceiling).consoleAutoResolve` (`bin/lib/issues/autonomy.js`, granted at `unattended` only — see `_shared/autonomy-ceiling.md`). When **not** granted, skip this section entirely and proceed to "Present a real stop" below — the ordinary `supervised`/`trusted`/interactive path, unchanged. When `--dry-run` was also passed, its preview-only behavior (above) takes precedence regardless of ceiling — nothing here executes a real write.
...
- Every batch-section item (Auto-applied through Cleanup actions) resolves as if "Approve all" had been chosen. **The merge half of that decision defaults to merge** (`integration-model: pr-first`'s "Approve all + merge" variant, never "leave PR open") — `unattended` authorizes full completion, not a park; `local-merge` proceeds to branch-finish the same way. **Needs-human carve-out (merge-check precedence):** when an `assess-agent-autonomy` `merge-check` verdict of `needs-human` exists for this run (any member's verdict, for a bundle — `dispatch/settle-and-merge.md`'s Auto-merge gate, Layer 2), the merge half instead resolves to **leave the PR open** (`pending-review`), never merge — `consoleAutoResolve`'s default-merge never overrides the more specific gate's routing to a human. **Ungranted-member carve-out (#1802):** the same leave-open resolution also applies when any group member's live labels lack both `auto:merge` and a matured `auto:merge-pending` — **"group member" here includes a solo, non-bundle record**: a singleton run's own single record is a one-member group, not an exemption, so an `auto:build`-only record with no `auto:merge` never merges under this carve-out regardless of whether it arrived alone or as part of a dispatch bundle (#1966 — the carve-out's original wording, phrased only in bundle terms, left this ambiguous). For a bundle, the Auto-merge gate's Layer 1 (`dispatch/settle-and-merge.md`) falls through before Layer 2's `merge-check` ever runs on a mixed-grant group, so no verdict exists to carry the precedence above; a withheld grant is itself a human decision and must never silently resolve to merge for want of a verdict that was never computed. **Drain-overlap-hold carve-out (#2299):** the same leave-open resolution also applies when
...
next evaluation. Every non-merge item still auto-resolves exactly as this section states.
...
Resolve every item in one process instead of one turn each (#1932): `node "${CLAUDE_PLUGIN_ROOT}/bin/console-resolve.js" --run "$PIPELINE_RUN_DIR" --policy console-auto` (append `--dry-run` when this wrap-up runs `--dry-run` — it then prints without writing). The CLI applies exactly the stances above to every staged item (`bin/lib/console/resolve.js`'s `SECTION_STANCES`, its prefix table `SECTION_MAP`; a staged patch is `git apply --check`ed first and resolves to `stale — re-derive from Invariant:` when it no longer applies, never applied blind, per `_shared/staged-patch.md`; an unrecognized staged prefix lands in Pending review as `pending — unmapped-prefix`, never auto-approved; a staged item named by a `REFUSED` line in `decisions.md` is classified under **Refused — no defer reason** before any prefix rule runs and resolves to `refused` — those rows are in the CLI's table like every other row, and a `refused` item is never applied, at any tier, per `refused-proposals.md`), computes the merge half with both carve-outs above (`merge.resolution` = `merge` | `leave-open`, with the reason — it **never** runs the merge itself), appends one decisions block (`AUTO {time} — Review Console: Console auto-resolved {n} item(s) at unattended (console-resolve.js). Reversibility: per item.` plus one line per item), writes `{run-dir}/console.json` (`{resolved: true, mode: 'auto-resolve', at, ceiling, items, merge}` — the write the reconciler's archival needs, #1854), and prints the console table once — every row stamped `AUTO-RESOLVED`. Render that table verbatim. Then execute every returned resolution through this file's own "On approval (option 1)" procedure below — the shared one `review-console-interactive.md` also defers to. Each resolution routes to exactly one step; no row in the table is left unrouted:

- `applied` — execute **nothing**. It is already in a commit; the row exists only so an override can revert it.
- `apply` on a **Pending review** item — step 1, which covers every staged item 5–7, not only the patches: a `.patch` via `_shared/staged-patch.md`'s `git apply` (a stale one takes the `Invariant:` re-derivation below), and a staged `.md` proposal (`polish-suggestion-*`, `visual-review-*`, `design-decision-*`, `build-deviation-*`, `simplify-*`, `deepen-*`) applied as that step already describes for a non-patch item.
- `apply` on a `Q#` item — step 7 (record creation), whose `refused-proposals.md` check still runs first; on an `M#` item — step 8 (the `_shared/learning-routing.md` memory write plus its `MEMORY.md` index line).
- `approve` — the curation steps: step 2 (Skill updates), step 3 (Documentation updates), step 4 (Journey updates), step 5 (Configuration updates). A **Reference repairs** row has no numbered step of its own — it is applied by Phase 4's execution step, per `execution-and-verification.md`'s Reference repairs bullet. Cleanup actions run at step 6.
- `filed` — step 9's `/claude-tweaks:feedback --pre-confirmed` filing (the `U#` divergence stated above).
- `stale` — still step 1, but its re-derivation path only: rebuild the diff from the item's `Target:`/`Invariant:` preamble. Never `git apply` a diff the CLI already reported as failing `--check`, and never drop it silently.
- `keep-staged`, `pending`, and `refused` — execute **nothing**. They stay in `staged/`, are named in the closing pointer below, and are never applied; `refused` rows have no default at any tier (`refused-proposals.md`).

The merge half is not one of those rows: run it separately per `merge.resolution` (`_shared/pr-first-merge.md` under `pr-first`; branch-finish under `local-merge`). **Retain `staged/` files** rather than deleting them — applied ones included (they stay as revert artifacts, the same way the auto-merge short-circuit's own commit is still revertible).

Non-zero exits: exit code 4 (ceiling not granted) → skip this section and proceed to "Present a real stop" below, the ordinary path; exit code 5 (this run's `console.json` is already a console rendered on the PR and awaiting a human, or it will not parse — the CLI wrote nothing either way) → also proceed to "Present a real stop", the human path: a rendered console is somebody's open decision, and no flag overwrites it; exit codes 2 and 3 (malformed invocation / `--run` not anchored) → a HARD-GATE failure: stop the wrap-up and report the CLI's stderr — an unattended run has nobody to answer a real stop. Send one consolidated `PushNotification` summarizing the run, at the same point the auto-merge short-circuit sends its own FYI (`_shared/autonomy-ceiling.md`'s Notification section) — one notification for the whole run, never one per item. End with the absolute path to `decisions.md`, `console.json`, and any retained `staged/*` files, so the operator has a concrete pointer even though nothing prompted at `unattended`.
...
10. Write `verify-expectations.json` in `$PIPELINE_RUN_DIR`: `{"version": 1, "memory": [...], "upstream": [...]}` — `memory` holds one `{file, indexFile}` entry per `M#` item resolved to Apply in step 8 (the file/indexFile the staged item named), empty array when none resolved to Apply this run; `upstream` holds one `{url}` entry per `U#` item resolved to filed in step 9, empty array when none resolved to filed this run. **When no materialized header exists under `${RUN_DIR}/work/*-spec.md`** (a current-branch, non-materialized record run — the same absent-header condition `engine-verify.js`'s `resolvedIssueNumbers()` falls back on before it will ever read this file's `issues` key), also include an `issues` array holding this run's own identified record number(s) from Phase 1 (the `#`-prefixed argument, a branch/commit reference, or a multi-record current-branch run's resolved set): `{"version": 1, "memory": [...], "upstream": [...], "issues": [...]}`. When a materialized header does exist, omit the `issues` key entirely — `resolvedIssueNumbers()`'s header-read path already takes priority and would never reach this key anyway. Always write this file, even when `memory`/`upstream` are both empty — an absent file and an empty one are DISTINCT states to the `verify` verb (`engine-verify.js`): absent means this write step itself didn't run, empty means it ran and found nothing to record. Skipped entirely under `--dry-run` — consistent with "no memory file is ever written" under `--dry-run` above.
...
If `decisions.md` holds no decision-bearing entries (`AUTO` / `STAGED` / `KEPT-PROMPT` / `REFUSED` / `FAILED` — `SCANNED` audit lines are excluded, see below; `SKIP` audit lines are also excluded, see below) AND `staged/` is empty AND there are no skill/config updates AND no cleanup actions apply AND no queue writes, memory updates, or upstream feedback proposals are pending, skip the console entirely. Write an empty `verify-expectations.json` (`{"version": 1, "memory": [], "upstream": []}`) in `$PIPELINE_RUN_DIR` if it does not already exist — nothing was resolved this run, so `memory-updates`/`upstream-feedback` should read "nothing recorded," not "file missing." Log "Review Console: nothing to review" and proceed to the phase-trace report.

Three kinds of entry never count toward this test, alongside each other:
...
- **`SKIP` audit lines** do not count as decision-bearing either, for the same reason: `SKIP` traces a documented conditional action that was skipped or degraded — nothing staged, nothing to act on (`_shared/auto-decision-log.md`'s Degrade-trace rule). A run that both skipped a step and found a real decision still writes its own separate `AUTO`/`STAGED`/`KEPT-PROMPT` line alongside the `SKIP` line, so this exclusion only widens the skip to a `decisions.md` holding purely `SCANNED`/`SKIP` lines, never a run that also found something.
