...
| `dispatch-fastlane-bundle-cap` | `policy.yml` | `/claude-tweaks:dispatch` | `3` | Caps how many non-overlapping `ceremony:fast-lane` singleton groups `queue-pull-script.md` folds into one multi-spec group after file-overlap grouping, sharing one preflight/wrap-up tail across the bundle instead of paying it per record — `0` turns bundling off, reproducing pre-#1910 grouping exactly |
...
| `design-variant-exploration` | `policy.yml` | `/claude-tweaks:specify` Step 2.5b-ii | `off` | `off`/`offer` — whether Step 2.5b-ii's layout-tournament (`design-wrapper explore`) and single-scaffold `live` offers fire at all during decomposition. `off` (the default) skips Step 2.5b-ii entirely, formalizing the already-observed behavior on a current Impeccable pin, where the tournament's own exact-pin availability check never passes anyway (`impeccable-plugin.md`'s Resolution). At `offer`, `explore` is presented only once the same exact-pin check has passed — an off-pin install renders as unavailable, naming both the installed and pinned versions, never as "Recommended" |
...
| `design-ceremony` | `policy.yml` — no run dir exists at the check point (brainstorming completes before any pipeline run starts, same timing as `specify-auto-continue` above) | `/claude-tweaks:specify` (its three `/superpowers:brainstorming` invocation sites) and `/claude-tweaks:capture`'s `--route=brainstorm` (#2347) | `standard` | `fast-lane`/`standard` — on `fast-lane`, the calling skill prepends one consolidation sentence to the `/superpowers:brainstorming` Skill-tool call's `args`, asking the Architectural path to present its remaining design sections as one consolidated block once the decision-carrying questions are answered and the approach is chosen, instead of gating each section — brainstorming's own final approval gate is never removed. Composed via `bin/compose-brainstorm-args.js`; full procedure in `specify/brainstorming-ceremony.md`. **Naming note:** this key's `fast-lane` value is an unrelated, coincidental token collision with `ceremony-profile`'s own `fast-lane` value (`_shared/ceremony-profile.md`, a diff-derived per-run concept) — see that file's own note for the accept-and-document decision and `tests/ceremony-profile-roster.test.js`'s adjacency heuristic this collision triggers |
...
| `backlog-fetch-limit` | `policy.yml` | `/claude-tweaks:help`, `/claude-tweaks:tidy`, `/claude-tweaks:backlog` | `1000` | Cap on `gh issue list --limit` for every `_shared/record-queue-fetch.md` consumer — `gh` auto-paginates internally; this bounds how many rows before a truncation warning fires, not a hard cutoff on backlog size. The limit bounds closed-record coverage, never open completeness — when the `--state all` union hits the cap, `record-queue-fetch.md`'s read-fresh-or-fetch block completes the open set with a second `--state open` fetch (#1919), so the *open* set stays complete even when closed-record coverage is truncated at the newest `{limit}` |
...
| `gh-timeout-ms` | `policy.yml` | `bin/lib/shared-primitives.js` (every `gh` subprocess call routed through its `GH_TIMEOUT_MS` export or its `runClassified`/`runClassifiedAsync` runner functions) | `5000` | Bounds (1000-60000) how long one `gh` subprocess call may run before it is killed. A call routed through `runClassified`/`runClassifiedAsync` with `retryOnTimeout: true` (`bin/lib/reconcile/release-merged.js`, `preflight.js`, `pr-state.js`) is retried exactly once at the same bound before its caller sees a failure; every other `GH_TIMEOUT_MS` consumer just uses the resolved bound directly, with no retry. `CLAUDE_TWEAKS_GH_TIMEOUT_MS` (an env var, not a policy key) takes precedence over this when both are set. Resolved once per process and cached — a project on GitHub Enterprise Server, where a cold `gh` call routinely exceeds the 5s default, raises this instead of every call failing until `gh` warms up |
...
| `port-services` | `policy.yml` | `bin/lib/hooks/session-start.js` (`bin/lib/ports/ensure.js`) | `[]` (empty — port isolation off) | Comma-separated service names naming this checkout's own dev-server processes — the first maps to `PORT`, each later one to `{NAME}_PORT` (sanitized upper-snake-case), all offsets from one 10-port block SessionStart leases via `bin/lib/ports/registry.js` (#1791) and writes into `.env.local`/`.env`. Not an auto-mode lever — read directly by the hook, never a Manifesto question. An entry failing `^[a-z][a-z0-9-]*$` is dropped and reported, not fatal; more than 10 (the registry's `BLOCK_SIZE`) valid entries cannot fit one block and disables port isolation for the session entirely. The managed region's first line is `CLAUDE_TWEAKS_LEASE={base}` (#1927) — a stable per-checkout token a project may key non-port resources on, e.g. a test database name. |
| `release-hook` | `policy.yml` (seeded commented-out by `/claude-tweaks:init` Step 21) | `bin/release-local.js` (unit 4 of #2250) | unset | Shell command the local engine runs after `v{version}` is tagged — the project's publish/mirror/deploy hook. Not read under `pr-first`, where the `release: published` workflow is the hook (needs a PAT — `GITHUB_TOKEN`-created releases do not trigger it). Not a Manifesto lever — write it unquoted or in one matched pair of quotes; a `#` ends the value |
