...
**2026-09-14 addendum (#1785):** five further false-positive shapes reproduced live during that record's review+wrap-up execution, plus two read-only shapes independently filed in #1867 and surfaced during the same build. Confirmed against plugin version 6.114.0.

- Quoted JSON containing `git`: `printf '%s' '[{"finding":"rollback leaves the git index staged"}]' > "$CTX/lens-3c.json"` — refused: "names git in a form too complex to verify". **Workaround:** use the `Write` tool instead: `Write({file_path: "/absolute/path", content: '[{"finding":"rollback leaves the git index staged"}]'})`.
- `for`/`while` loop variable: `for f in docs/journeys/*.md; do sed -n '/^files:/,/^---/p' "$f"; done` — refused: "runs sed with a value computed at runtime (the variable f)". **Workaround:** iterate via separate plain commands instead: invoke `sed -n '/^files:/,/^---/p' docs/journeys/a.md` on one call, then on a second call `sed -n '/^files:/,/^---/p' docs/journeys/b.md`, or use `find ... -exec sed ...` if multiple files can be processed in one invocation without a loop token.
- Glob argument: `find <plugin-cache-dir> -maxdepth 3 -iname "*impeccable*"` — refused: "runs find with a value computed at runtime (a glob pattern)". **Workaround:** expand the glob in a separate literal form or use `find` with fixed-string matching: `find /path/to/dir -maxdepth 3 -name impeccable`.
- `gh api` call inside `if RAW=$(...)`: `gh api "repos/{owner}/{repo}/contents/claims/issue-N.json?ref=claims-registry" -q ...` inside `if RAW=$(...); then` — refused: "runs gh with the text repos/…/contents… inside a construct too complex to verify". **Workaround:** move the `gh` call to a separate plain command on its own, store the result in a file, and read it back on a second call: first invoke `gh api "repos/{owner}/{repo}/contents/claims/issue-N.json?ref=claims-registry" -q > "$TMPFILE"`, then on a second Bash call run `RAW=$(cat "$TMPFILE")` and the `if` logic.
- `sed -i` on a variable path: `sed -i 's/^- \[ \] review$/- [x] review/' "$B"` where `$B` is a scratch-file path variable — refused: "runs sed with a value computed at runtime (the variable B)". **Workaround:** use the `Edit` tool instead with the literal file path substituted in: `Edit({file_path: "/absolute/path/to/file", old_string: "- [ ] review", new_string: "- [x] review"})`.
- `cat "$P/…"` with a runtime-computed path variable: `cat "$P/subdir/file.md"` where `$P` holds an absolute path assigned earlier — refused: "runs cat with a value computed at runtime (the variable P)". **Workaround:** use the `Read` tool instead: `Read({file_path: "/absolute/path/subdir/file.md"})`.
- `sed -n 'a,bp' "$P/…"` with a runtime-computed path variable: `sed -n '10,20p' "$P/subdir/file.md"` where `$P` holds an absolute path — refused: "runs sed with a value computed at runtime (the variable P)". **Workaround:** use the `Read` tool with offset/limit instead: `Read({file_path: "/absolute/path/subdir/file.md", offset: 9, limit: 11})`.
