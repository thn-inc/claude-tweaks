...
9. **Context-cost bloat scan** (any kind). Run the mechanical detector over the target, with the target's siblings supplying the corpus baseline:
   ...
   It reports four signals: files over the 45 KB soft ceiling, Anti-Pattern rows more than twice the corpus median byte length, provenance-narration phrasing (text addressed to whoever edited the file rather than to the model running it), and adjacent table rows whose right-hand cells are identical or near-identical. Every reported line is evidence dimension 9 weighs, never a finding on its own — see the bloat-signal guard in Step 2 for what each signal's legitimate form looks like. When the corpus yields fewer than 20 Anti-Pattern rows the report prints `NO BASELINE` and the row signal was **not evaluated**: read that as "not checked," never as "clean."
10. **Composed-bytes measurement** (skills only). Run `composedBytesReport` from `${CLAUDE_PLUGIN_ROOT}/bin/lib/skill-audit/context-cost.js` over the plugin root and read the per-call-site table it returns (one row per compose call site, one column per condition combination its sources branch on):
    ```bash
    node -e "
    const { composedBytesReport, overComposedCeiling } = require('${CLAUDE_PLUGIN_ROOT}/bin/lib/skill-audit/context-cost.js');
    const rows = composedBytesReport(process.argv[1]); console.log(JSON.stringify({ rows: rows.map((r) => ({ step: r.step, file: r.file, max: r.max, error: r.error })), over: overComposedCeiling(rows) }));
    " "<plugin-root>"
    ```
    `<plugin-root>` is the directory with `skills/` directly beneath it — this repo's `plugin/`, or `${CLAUDE_PLUGIN_ROOT}` for an installed consumer — never the repo root. A combination over `CEILING_BYTES` — or a step carried by `COMPOSED_STEP_EXCEPTIONS` — is evidence for dimension 9, cited by the module's numbers, never restated here.
...
| **9. Context-cost bloat** | Is the document paying context for text that instructs nobody — an oversized file, rows grown into paragraphs, narration of its own edit history, a table spending N rows on one fact, or a compose call site composing over its ceiling? (Use Step 1's checks 9 and 10 as the evidence.) | ✓ (all four signals, plus check 10's composed-bytes gate) | ✓ (provenance and degenerate-row signals; size is check 4's tiered line budget, not the byte ceiling) | ✓ (same as rule) |
| **10. Model-generation fit** (new, #2656) | Does this rule/section read like scaffolding written for an older Claude generation's limitations — verbose defensive repetition, an explicit instruction for something the currently-targeted model already does by default — rather than something the current generation still needs spelled out? (See the Model-generation-fit guard below; cross-references `#2644`.) | ✓ | ✓ | ✓ |
...
**Bloat-signal guard (dimension 9).** Every signal Step 1's checks 9 and 10 report has a legitimate form, so none is a finding on its own. Read the flagged line before emitting anything:

- **Over the ceiling.** A real finding for a skill file or one of its sub-files: it loads in full on every invocation and once per dispatched subagent. The fix is composition first — `when:` markers or a compose call site whose composed bytes the gate measures (check 10) — and extraction to a lazy-loaded sub-file only when the sub-file is a genuinely lazy unit some runs never read; never raising the ceiling. A sub-file cited by two or more stubs naming *sections* of it is the same defect one level down, since `Read` has no section granularity. For a `rule` or `claude-md` target the byte-ceiling line restates check 4's tiered line budget; bundle them, don't file both.
...
Nothing in this dimension licenses an edit: a bloat finding proposes the exact `oldString`/`newString` that removes the weight and files like any other, and it owes the same Anchor Requirement — cite checks 9 and 10's reported line and byte count, never an impression that the file feels long.

**Model-generation-fit guard (dimension 10, #2656).** A different axis from dimensions 7-9 above: those judge a document against the codebase's current reality or its own structural template; this one judges a document against the *currently-targeted Claude model's* actual behavior — whether an instruction is still load-bearing, or whether it was written to compensate for a limitation a newer model generation no longer has (verbose defensive repetition, an explicit step-by-step breakdown of something the model now does unprompted, a workaround for a formatting/tool-use quirk since fixed). This premise — that a newer model needs measurably less scaffolding — is a hypothesis to probe per rule, not a mechanical rule to apply across the board; a rule can still be load-bearing for a newer model for reasons that have nothing to do with verbosity (project-specific conventions no model would infer on its own, a genuine safety/correctness guardrail, a hazard this file's own "Rule expiry" check would independently confirm is still live). Do not conflate the two checks: rule expiry (above) asks "can the hazard this rule guards still happen at all," a codebase/environment question; this dimension asks "does explaining it still need this many words for this model," a prompting-economy question — the same rule can pass one and fail the other, and a finding here must never restate rule-expiry's own reasoning as if it were evidence of staleness.

Judging this is inherently softer than harness-health's other, more mechanical dimensions (Step 1's checks are deterministic; this one asks the auditing model to reason about its own capabilities relative to a prior generation) — expect a lower default `confidence` than dimensions 1-9 produce, and treat every finding here as needing more human review before consolidation, never a candidate for an unattended-apply path (harness-health already never auto-applies to any target — this dimension does not change that posture, it only makes the review bar explicit).

**Overlap with dimension 8 (best-practice/harness-performance fit).** The two can flag the same passage for different reasons — dimension 8 asks whether a passage follows known prompting practice in general (clear triggers, no cross-skill overlap, right-sized scope); this dimension asks specifically whether it is padding relative to what *this* model generation needs. When both would fire on the same span, file one finding under `category: "best-practice"` and note in `reason` that it is also a model-generation-fit case, rather than double-filing the same span from two dimensions.

**Trigger — shared with `#2644`.** This dimension and `#2644` (model-version prompting-delta tracking for skill-authoring guidance) are both triggered by the identical event: a new Claude model generation shipping. Neither is wired into harness-health's own churn/staleness rotation cursor (`next-target.js`) as an automatic due-ness signal — a model-version release is not a repo-churn event that cursor can detect, and building that detection is out of scope here. Re-run this dimension by hand (`/claude-tweaks:harness-health --target <id>` against candidate skills/CLAUDE.md, or a `--budget`-driven sweep) whenever `#2644` is re-opened or a similar model-version-prompting-deltas record is filed, rather than treating either as a standalone, uncoordinated process. Category is always `best-practice` (the document is accurate; it's the prompting economy that's stale, the same reason dimension 8 uses that category).
