...
**Value-collision decision (#2347).** A second, unrelated static policy key — `design-ceremony` (`policy-schema.md`), controlling whether `/superpowers:brainstorming` consolidates its design sections at `/specify`'s three call sites plus `/claude-tweaks:capture`'s `--route=brainstorm` — happens to share this file's own `fast-lane` value token, despite governing a genuinely different mechanism (brainstorming section consolidation, not skip-a-step). **Decision: accept the collision, do not rename either key.** A rename of `design-ceremony`'s `fast-lane` value is a breaking change to its own shipped acceptance criterion and needs explicit human sign-off before landing (never done unilaterally by an autonomous build); this record's own filing defaults to accept-and-document absent that sign-off. The concrete symptom this collision produces: `tests/ceremony-profile-roster.test.js`'s line-level heuristic above pairs the literal words `fast-lane` and `skip` regardless of which mechanism a line is actually about, so any skill-prose line describing `design-ceremony`'s `fast-lane` value using the word "skip" nearby false-positives against this roster's test. `specify/brainstorming-ceremony.md` and `capture/routing.md` both work around this today by keeping their `design-ceremony`/`fast-lane` mentions off any line containing `skip` — read `specify/brainstorming-ceremony.md`'s own header note before rewording either citation.
