...
| `_shared/pr-first-merge-post-merge.md` (Step 4.1) | `release-status` | Body: the resolved value (`v{tag}` or `unreleased`) from `git describe --tags --contains --first-parent {merge-sha}`; posted under `pr-first` on either outcome |
