...
| **10. Model-generation fit** (#2656) | Does this read like scaffolding for an older Claude generation's limitations — defensive repetition, an explicit breakdown of something the current model does unprompted — rather than something it still needs spelled out? | ✓ | ✓ | ✓ |
...
**Model-generation-fit guard (dimension 10, #2656).** A different axis from 7-9: those judge a document against the codebase or its own template; this judges it against the *currently-targeted model's* actual behavior — is an instruction still load-bearing, or a workaround for a limitation a newer generation no longer has. Treat "newer models need less scaffolding" as a hypothesis to probe per rule, not a mechanical sweep — a rule can stay load-bearing for reasons unrelated to verbosity (project-specific conventions, a genuine guardrail the rule-expiry check below would independently confirm is still live). Distinct from rule expiry: expiry asks whether the guarded hazard can still happen at all (a codebase question); this asks whether explaining it still needs this many words for this model (a prompting-economy question) — the same rule can pass one and fail the other. Expect lower default confidence than dimensions 1-9 (this asks the auditing model to reason about its own capabilities relative to a prior generation) and more human review before consolidation. Overlaps dimension 8 in scope — when both would fire on the same span, file one finding under `category: "best-practice"` rather than double-filing. Not wired into this skill's own churn/staleness rotation; re-run by hand whenever `#2644` (model-version prompting-delta tracking) is re-opened — the two share the identical triggering event (a new model generation shipping) and must stay coordinated rather than drift into separate processes. A finding proposes a consolidated rewrite, never an automatic deletion, naming the specific older-model quirk the flagged text appears to compensate for.
