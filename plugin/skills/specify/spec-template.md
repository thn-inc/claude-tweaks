...
- {A Current-State/Key-Files claim `_shared/premise-verification.md`'s check could not settle within its caps, or that is genuinely unverifiable from this checkout — literal marker, not paraphrased: `ASSUMPTION — verify at build: {claim} — confirm via {what would settle it}.`}
...
- A `## Current State`/`### Key Files` sentence asserting a fact about existing code (a file/module/symbol exists or is absent, or a module reads/writes/calls a named field) with neither an inline evidence citation nor the `ASSUMPTION — verify at build:` marker (`_shared/premise-verification.md`) — an unverified claim written in the same declarative voice as a confirmed one is exactly the gap record #1769 closes
