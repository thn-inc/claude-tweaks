...
held at the tip of `main` —
the marketplace `source` is an unpinned git URL, so an install tracks that tip,
and every distinct value it reported is a build someone could be running.
