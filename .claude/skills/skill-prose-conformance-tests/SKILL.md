...
### An ordering claim's control must carry both markers — in the wrong order

`tests/verification-skip-before-execute.test.js` (#2527) and `tests/design-variant-exploration-availability-order.test.js` (#2540) both pin "section X comes before section Y" with plain `indexOf` position comparisons over the live file. That is the right instrument when the property is *which comes first* rather than whether a literal is present — but it carries a blind spot the presence, count, and adjacency rules above do not cover. A `requireOrder`-style helper asserts each marker exists (`assert.notStrictEqual(idx, -1)`) and only then that positions increase, so **the existence half carries the entire go-red proof whenever a marker is new in the same change**.

#2540 is the worked example: all three of its ordering tests pin markers that do not exist at the base ref — `## Availability (exact-pin, checked before any option is presented)` in `design-wrapper/modes/explore.md`, and `**Policy gate.**` / `**Exact-pin availability pre-check.**` in `specify/design-pre-steps.md` (confirm with `git show {base}:{file} | grep -c -F '{marker}'` — all zero). Every one of them goes red at base on the *missing marker*, and the `idx > last` comparison the suite exists for was never exercised. This is not a vacuous pin — the comparison does catch the regression it guards, a later edit moving the section back below the offers — it is an **unexercised** one: nothing shows the helper's ordering arithmetic works at all, and a `requireOrder` that compared the wrong variable, or reset `last` each iteration, would be exactly as green.

Build the control by **permutation, not deletion**: take the live text, splice the later block above the earlier one, and assert the same helper *throws* on the permuted string — one `assert.throws` per ordered pair, in the same suite. Where the two sections have clean boundaries, the blank-line paragraph split from "Doctor by structural boundary" below yields the permutation directly, and the exact-count guard there applies unchanged. A frozen pre-change excerpt is the wrong control here for the same reason it is wrong for a new adjacency window (the blind spot above): at base one marker is simply absent, so the failure is attributable to absence rather than to order. This is also why the `grep -c -F` recipe in "Proving discrimination without editing the tree" does not settle an ordering pin — it is presence-shaped, and "one marker missing at base" is precisely the reading that proves nothing.
...
**An empty extraction passes the sweep for free.** `assert.deepStrictEqual(offenders, [], …)` over a derived haystack is green when the extractor returns nothing — a fence style the regex misses (`~~~`, an indented block), a renamed target read as an empty string, or a typo. This is the `doesNotMatch` blind spot above one level up: there the *control* lacked the tokens, here the *haystack* does. It is live, not hypothetical: `multispec-freshness.md` today has zero fenced blocks and dozens of inline spans (count live: `grep -oE '`[^`\n]+`' plugin/skills/flow/multispec-freshness.md | wc -l`), so the fence half of that helper is currently pinning nothing and a bug in it would not show. Assert the region count is non-empty, and run the same filter over a frozen string carrying the banned literal, so green proves the sweep can still see one.
...
| Which of two sections comes first — an ordering claim, not a presence one | Compare `indexOf` positions over the live file, then prove the comparison by permuting the live text (splice the later block above the earlier) and asserting the helper throws. A frozen pre-change excerpt in which one marker does not exist yet proves absence, never order |
...
- **A live-probe negative control substitutes a nonsense token, never a plausible real path.** The go-red controls above are frozen bytes, so they are stable by construction; a control that instead runs the extracted snippet against the *live* repo and asserts zero hits is only as stable as repo state. Substitute a plausible-looking token — a real `_shared` filename, a module name someone might reasonably add — and the assertion is a false red waiting on any future commit, and it reads as a product regression rather than as a stale control. Pick a token structurally unlikely to ever match (`zzz-no-such-generator-1321.md` in `tests/spec-template-generator-grep.test.js`), and say in the comment beside it that a hit means re-pick the control, not that the shipped procedure broke. #1321's first control was `feedback-objectives.md`, a live `_shared` file; final review replaced it (`5f19358ed`).
...
| Proving an ordering pin's go-red against a base snapshot where one of the markers does not exist yet | The missing-marker assertion fires first, so the position comparison the suite exists for is never exercised and a helper comparing the wrong variable stays green. Permute the live text instead (`tests/design-variant-exploration-availability-order.test.js`, #2540 — all three ordering pins land this way) |
